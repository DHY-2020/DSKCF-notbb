...
cv::Mat1i DepthSegmenter::init( const cv::Mat & image, const Rect & boundingBox )
{
	...
	if( getSubWindow( image, front_depth, windowSize, windowPosition  ) )
	{
		...
		// cv::minMaxLoc( front_depth, &min_depth, &max_depth, nullptr, nullptr, mask );
		// cout<<1<<endl;
...
int DepthSegmenter::update( const cv::Mat & image, const Rect & boundingBox )
{
	...
	if( getSubWindow( image, front_depth, windowSize, windowPosition  ) )
	{
		...
		//Find and store the empty depth values to be excluded from the histogram 查找并存储要从直方图中排除的空深度值
		cv::Mat1b mask = createMask( front_depth );
	...
	// cout<<"peaks.size:"<<peaks.size()<<endl;
		...
		if( peaks.size() > 0 && (emptyDepth==false))
		{
			...
			//Find the nearest object and calculate its mean depth and standard deviation找到最近的物体并计算其平均深度和标准偏差
			...
			int binCenter=cvRound(centerDepth);//the center are already in bin coordinates中心已经在 bin 坐标中
			...
			// int tmpBin=(this->m_histogram.depthToBin( this->getTargetDepth() - this->getTargetSTD() ));
			// cout<<"bin:"<<bin<<endl;
			// cout<<"tmpBin:"<<tmpBin<<endl;
			bin = std::min< int >( bin, tmpBin );
			// bin = std::max< int >( bin, tmpBin );

...
const std::vector< int > DepthHistogram::getPeaks() const
{
  ...
  // return this->getPeaks( ( this->size() > 50 ? 3 : 1 ) );
}
...
const std::vector< int > DepthHistogram::getPeaks( const int minimumPeakdistance, const double minimumPeakHeight ) const
{
  if( !this->m_bins.empty() )
  {
    ...
    //cout<<"minimumPeakHeight:"<<minimumPeakHeight<<endl;
    //cout<<"minimumPeakdistance:"<<minimumPeakdistance<<endl;

    cv::minMaxLoc( this->m_bins, nullptr, &max );
    cv::minMaxLoc( local_histogram, nullptr, &current_max, nullptr, &current_max_location );

    //Check if the first bin is a peak (special case, no left neighbour)检查第一个 bin 是否是峰值（特殊情况，没有左邻居）
    if( ( this->m_bins( 0 ) > this->m_bins( 1 ) ) && ( this->m_bins( 0 ) > minimumPeakHeight * max ) )
    {
      candidates.push_back( 0 );
    }

    //Check if the last bin is a peak (special case, no right neighbour)检查最后一个 bin 是否是峰值（特殊情况，没有右邻）
    ...
    //Check the rest of the bins to see if they are a peak检查其余的 bin 以查看它们是否为峰值
    ...
    //Filter out the neighbouring peaks that are closer than the minimum peak distance过滤掉比最小峰距离更近的相邻峰
...
const DepthHistogram::Labels DepthHistogram::kmeans( const std::vector< float > & centroids ) const
{
  ...
  while( dC > 1.0f )
  {
    ...
    for(uint i=0; i<this->m_bins.rows;i++)
    {
...
const DepthHistogram DepthHistogram::createHistogram( const uint step, const cv::Mat & region, const cv::Mat1b & mask )
{
  ...
  // region.copyTo( region32f);
  cv::minMaxLoc( region32f, &result.m_minimum, &result.m_maximum, nullptr, nullptr, mask );
  //cout<<"step:"<<step<<endl;
  //cout<<"result.m_minimum:"<<result.m_minimum<<endl;
  //cout<<"result.m_maximum:"<<result.m_maximum<<endl;
  // cout<<"mask:"<<mask<<endl;
  ...
  // result.m_minimum += modelNoise( result.m_minimum, 0 );
  ...
  //计算直方图 1.输入的图像数组 2 输入数组的个数 3通道数 4掩码 5直方图 6直方图维度 7直方图每个维度的尺寸数组 8每一位数组的范围

...
void KcfTracker::onScaleChange( const Size & targetSize, const Size & windowSize, const cv::Mat2d & yf, const cv::Mat1d & cosineWindow )
{
  ...
  if( this->m_isInitialized )
  {
    ...
    // cout<<"modelsize:"<<modelSize.area()<<endl;
    for(int index=0; index<this->m_xf->channels.size();index++)
    {
      cv::Mat & channel = this->m_xf->channels[index];
      
      channel = ScaleAnalyser::scaleImageFourierShift( channel, modelSize );
      // cout<<"channel:"<<channel.size()<<endl;
      

    }
    // cout<<"m_xf->channels:"<<this->m_xf->channels[0].size()<<endl;
